...
class TrackStatusController extends Controller
{
    ...
    public function actionStatus()
    {
        ...
        if ($trackStauts->load(Yii::$app->request->post() ) ) {   
            $travel_info_exist = TravelInfo::find()->where(['token'=>$trackStauts->token])->exists();
            $passenger_exist = Passenger::find()->where(['mobile_no'=>$trackStauts->mobile_no])->exists();
            // if($travel_info_exist)
            //     die("travel_info:".$trackStauts->token);
            // else
            //     die("no travel_info:".$trackStauts->token);
            // if($passenger_exist)
            //     die("mobile");
            if($travel_info_exist && $passenger_exist) {
                $model = TravelInfo::find()->where(['token'=>$trackStauts->token])->one();
                $passenger = Passenger::find()->where(['travel_info'=>$model->id])->one();
                return $this->redirect(['view', 'id' => $model->id,'passenger_id'=>$passenger->id]);    
            }
            else {
                Yii::$app->session->SetFlash("error","Could not find the staus");
